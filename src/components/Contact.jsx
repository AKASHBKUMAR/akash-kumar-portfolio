const socialLinks = [
  {
    href: 'https://github.com/AKASHBKUMAR',
  ...
  {
    href: 'https://www.linkedin.com/in/akash-kumar-b-71327425a/',
    icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <section className="mt-10 mb-20" id="contact">
      <div className="container lg:grid lg:grid-cols-2 lg:items-stretch">
        <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
          <h2 className="headline-2 lg:mxax-w-[12ch]">Contact me for collaboration</h2>
          ...
          <div className="flex items-center gap-2 mt-auto">
            {socialLinks.map(({href,icon},key)=>(
              <a href={href} className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-lg transition-[background-color,color] hover:bg-zinc-50 active:bg-zinc-50/80" key={key} target="_blank">{icon}</a>
            ))}
          </div>
        </div>

        <form action="https://getform.io/f/bqooogxb" method="POST" className="xl:pl-10 2xl:pl-20">
          <div className="md:grid md:items-center md:grid-cols-2 md:gap-2">
            <div className="mb-4">
              <label htmlFor="name" className="label">Name</label>
              <input type="text" name="name" id="name" autoComplete="name" className="text-field" required placeholder="Akash Kumar B"/>
            </div>

            <div className="mb-4">
              <label htmlFor="email" className="label">Email</label>
              <input type="email" name="email" id="email" autoComplete="email" className="text-field" required placeholder="example@gmail.com"/>
            </div>
          </div>

          <div className="mb-4">
            <label htmlFor="message" className="message">Message</label>
            <textarea name="message" id="message" placeholder="Hii!" required className="text-field resize-y min-h-32"></textarea>
          </div>
